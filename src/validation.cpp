...
public:
    ...
    std::vector<MempoolAcceptResult> AcceptMultipleTransactions(std::vector<const CTransactionRef>& txns, ATMPArgs& args) EXCLUSIVE_LOCKS_REQUIRED(cs_main);
...
private:
    ...
    bool PolicyScriptChecks(ATMPArgs& args, MempoolAcceptResult& result, Workspace& ws, PrecomputedTransactionData& txdata) EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_pool.cs);

    // Re-run the script checks, using consensus flags, and try to cache the
    // result in the scriptcache. This should be done after
    // PolicyScriptChecks(). This requires that all inputs either be in our
    // utxo set or in the mempool.
    bool ConsensusScriptChecks(ATMPArgs& args, MempoolAcceptResult& result, Workspace& ws, PrecomputedTransactionData &txdata) EXCLUSIVE_LOCKS_REQUIRED(cs_main, m_pool.cs);
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, MempoolAcceptResult& result, Workspace& ws)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        Coin coin;
        assert(m_view.GetCoin(txin.prevout, coin));
        const CTransactionRef& txFrom = m_pool.get(txin.prevout.hash);
        if (txFrom) {
            assert(txFrom->GetHash() == txin.prevout.hash);
            assert(txFrom->vout.size() > txin.prevout.n);
            assert(txFrom->vout[txin.prevout.n] == coin.out);
        } else if (m_viewmempool.PackageContains(txin.prevout.hash)) {
            // This input is provided by a transaction we are
            // considering for a package.
        } else {
...
bool MemPoolAccept::ConsensusScriptChecks(ATMPArgs& args, MempoolAcceptResult& result, Workspace& ws, PrecomputedTransactionData& txdata)
{
    ...
    if (!CheckInputScripts(tx, state, m_view, currentBlockScriptVerifyFlags, /* cacheSigStore = */ true, /* cacheFullSciptStore = */ true, txdata)) {
...
std::vector<MempoolAcceptResult> ProcessNewPackage(CTxMemPool& pool, std::vector<const CTransactionRef>& txns, bool test_accept)
{
    AssertLockHeld(cs_main);
    ...
    std::vector<MempoolAcceptResult> results = MemPoolAccept(pool).AcceptMultipleTransactions(txns, args);
